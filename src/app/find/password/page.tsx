...
const FindPasswordPage: React.FC = () => {
  ...
  const handleCloseModal = () => {
    ...
    if (modalTitle === "Verification Successful!") {
      router.push(`/find/password/reset?email=${submittedEmail}&token=${getValues("token")}`);
    }
  };
  ...
  const handleEmailSubmit: SubmitHandler<FindPasswordInput> = async (data) => {
    try {
      await axios.post(`${process.env.API_URL}/auth/password/request`, { email: data.email });
      setModalTitle("Request Successful!");
      setModalMessage("A verification code has been sent to your email.");
      setModalButtonColor("bg-green-600 hover:bg-green-500");
      setModalButtonName("Verify Code");
      setSubmittedEmail(data.email!);
      setFormState('token');
      setValue("email", data.email!);
    } catch (error) {
      if (axios.isAxiosError(error)) {
        if (error.response?.data === "This account uses SNS login.") {
          setModalTitle("Request Failed");
          setModalMessage("This account uses SNS login.");
  ...
  const handleTokenSubmit: SubmitHandler<FindPasswordInput> = async (data) => {
    try {
      await axios.post(`${process.env.API_URL}/auth/password/verify`, { email: data.email, token: data.token });
      setModalTitle("Verification Successful!");
      setModalMessage("Verification completed successfully.");
      setModalButtonColor("bg-green-600 hover:bg-green-500");
      setModalButtonName("Change Password");
      setIsModalOpen(true);
    } catch (error) {
      if (axios.isAxiosError(error)) {
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          {formState === 'email' && (
            <form onSubmit={handleSubmit(handleEmailSubmit)} noValidate>
              ...
              <div className="mt-6">
                ...
                >
                  {isSubmitting ? "Requesting..." : "Get Verification Code"}
                </button>
              </div>
            </form>
          )}
          ...
          {formState === 'token' && (
            <form onSubmit={handleSubmit(handleTokenSubmit)} noValidate>
              ...
              <div className="mt-6">
                <label htmlFor="token" className="block text-sm font-medium text-gray-700">Verification Code</label>
                <div className="mt-1 relative rounded-md shadow-sm">
                  <input
                    {...register("token", {
